...
function renderCartPage() {
  ...
  if (!tableBody || !cartEmpty || !summaryTotal)
    return;
  const cart = getCart();
  if (!cart.length) {
...
function submitCheckout(event) {
  ...
  const name = ((_a = document.getElementById("checkoutName")) == null ? void 0 : _a.value.trim()) || "";
...
function renderContactPage() {
  ...
  form.addEventListener("submit", (event) => {
    const _a, _b, _c;
    event.preventDefault();
    const name = ((_a = document.getElementById("contactName")) == null ? void 0 : _a.value.trim()) || "";
    const email = ((_b = document.getElementById("contactEmail")) == null ? void 0 : _b.value.trim()) || "";
    const message = ((_c = document.getElementById("contactMessage")) == null ? void 0 : _c.value.trim()) || "";
...
function handleAdminLogin(event) {
  const _a, _b;
  event.preventDefault();
  const username = ((_a = document.getElementById("adminUsername")) == null ? void 0 : _a.value.trim()) || "";
  const password = ((_b = document.getElementById("adminPassword")) == null ? void 0 : _b.value.trim()) || "";
...
async function handleAdminSave(event) {
  const _a, _b, _c, _d, _e, _f, _g, _h, _j;
  event.preventDefault();
...
function resetAdminForm() {
  ...
  if (adminSubmitButton)
    adminSubmitButton.textContent = "Add Product";
  if (adminCancelEdit)
...
function editProduct(productId) {
  ...
  document.getElementById("adminPrice").value = String(product.price);
  ...
  if (adminCancelEdit)
    adminCancelEdit.style.display = "inline-flex";
}
